...
def fill_white_list(random_choice, user_id):
    for item in random_choice:
        ...
        if not data:
            data = Favorite(fnd_user_id=item['id'], user_id=item['user_id'], user_name=item['first_name'],
                            user_surname=item['last_name'],
                            link=item['vk_link']
                            )
        session.add(data)
...
def check_db_favorites(user_id):
    ...
    if db_favorites:
        for item in db_favorites:
            all_users.append([item.user_id, 'id:' + str(item.fnd_user_id), item.user_name + ' ' + item.user_surname,
                              item.link + ' '])
        return all_users
